...
function App() {
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      if (error) throw error;
      if (data?.user) setSession((prev: any) => ({ ...prev, user: data.user }));
        ...
        !session ? <Login /> : (
          <div className={`relative flex flex-col h-[100dvh] transition-colors duration-500 ${isDarkMode ? 'bg-[#0f1115] text-slate-100' : 'bg-[#f4f5f7] text-slate-900'} font-sans antialiased overflow-hidden`}>
              ...
              <div className="max-w-6xl mx-auto py-4 flex flex-col lg:flex-row items-center justify-between gap-4">
                <div className="flex items-center justify-between w-full lg:w-auto">
                  ...
                  <div className="lg:hidden flex items-center gap-3">
                    ...
                    <div onClick={() => setIsEditingProfile(true)} className="w-9 h-9 rounded-xl border-2 border-brand-600/20 flex items-center justify-center cursor-pointer transition-all overflow-hidden bg-white dark:bg-[#0f1115]">
                      {session.user.user_metadata?.avatar_url ? <img src={session.user.user_metadata.avatar_url} alt="Avatar" className="w-full h-full object-cover" /> : <span className="text-[10px] font-black text-brand-600">{getInitials(session.user.user_metadata?.name || 'U')}</span>}
                    </div>
                ...
                <div className="hidden lg:flex items-center gap-4">
                  ...
                  <div onClick={() => setIsEditingProfile(true)} className="w-11 h-11 rounded-2xl border-2 border-brand-600/20 flex items-center justify-center cursor-pointer transition-all overflow-hidden bg-white dark:bg-[#0f1115]">
                    {session.user.user_metadata?.avatar_url ? <img src={session.user.user_metadata.avatar_url} alt="Avatar" className="w-full h-full object-cover" /> : <span className="text-sm font-bold text-brand-600">{getInitials(session.user.user_metadata?.name || 'U')}</span>}
                  </div>
                </div>
              </div>
            </header>
                  ...
                  <div className={`lg:col-span-4 space-y-6 ${(activeTab === 'summary' || activeTab === 'form') ? 'block' : 'hidden lg:block'} text-left`}>
                    <div className={`p-6 md:p-8 rounded-3xl border ${isDarkMode ? 'bg-[#161a20] border-slate-800 shadow-xl' : 'bg-white border-slate-200 shadow-sm'} relative overflow-hidden ${activeTab === 'summary' ? 'block' : 'hidden lg:block'} text-left`}>
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-slate-800/10 dark:border-slate-800">
                        <div><p className="text-[9px] text-slate-400 uppercase font-bold">Receitas</p><p className="text-emerald-500 font-black text-sm">{showBalance ? `+${summary.income.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '•••'}</p></div>
                        <div><p className="text-[9px] text-slate-400 uppercase font-bold">Despesas</p><p className="text-rose-500 font-black text-sm">{showBalance ? `-${summary.expense.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '•••'}</p></div>
                      </div>
                    </div>
                    <div className={`p-6 md:p-8 rounded-3xl border ${isDarkMode ? 'bg-[#161a20] border-slate-800 shadow-xl' : 'bg-white border-slate-200 shadow-sm'} ${activeTab === 'form' ? 'block' : 'hidden lg:block'} text-left`}>
                      <h3 className="font-black text-sm uppercase tracking-widest mb-6">Novo Lançamento</h3>
                      <form onSubmit={handleSaveTransaction} className="space-y-4">
                        ...
                        <div className="flex gap-2 p-1 bg-black/5 dark:bg-slate-900 rounded-2xl">
                        ...
                        {filteredTransactions.map((t) => (
                          <div key={t.id} className="p-5 flex items-center justify-between hover:bg-black/5 dark:hover:bg-white/5 transition-all">
                            <div className="flex items-center gap-4">
                              ...
                              <div><p className="font-bold text-sm uppercase tracking-tight">{t.description}</p><p className="text-[9px] text-slate-500 font-black uppercase">{format(parseISO(t.date), "dd 'de' MMMM", { locale: ptBR })}</p></div>
                            </div>
                            <div className="flex items-center gap-4">
                              ...
                              <button onClick={() => removeTransaction(t.id)} className="p-2 text-slate-400 hover:text-rose-500 transition-all"><Trash2 className="w-4 h-4" /></button>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <Dialog open={isEditingProfile} onOpenChange={setIsEditingProfile}>
              <DialogContent className="max-w-[380px] rounded-[40px] border-white/10 bg-[#0f1115] p-0 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                <DialogHeader className="p-6 border-b border-white/5 flex flex-row items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <DialogTitle className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50">Personalizar Perfil</DialogTitle>
                  </div>
                </DialogHeader>
                <div className="p-8 space-y-8">
                  <div className="relative w-32 h-32 mx-auto group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                    ...
                    <div className="relative w-full h-full rounded-[38px] border-2 border-white/10 overflow-hidden bg-black flex items-center justify-center transition-transform duration-500 group-hover:scale-105">
                      {session.user.user_metadata?.avatar_url ? <img src={session.user.user_metadata.avatar_url} alt="Avatar" className="w-full h-full object-cover" /> : <div className="flex flex-col items-center gap-1"><User className="w-8 h-8 text-brand-600" /><span className="text-[10px] font-black text-brand-600">{getInitials(session.user.user_metadata?.name || 'U')}</span></div>}
                      <div className="absolute inset-0 bg-brand-900/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Camera className="text-white w-6 h-6 mb-1" /><span className="text-[8px] font-black text-white uppercase tracking-widest">Alterar</span></div>
                    </div>
                    <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileChange} accept="image/*" />
                  </div>
                  <div className="space-y-4">
                    <div className="space-y-1"><label className="text-[9px] font-black text-white/30 uppercase ml-1 tracking-widest">Nome de Exibição</label><div className="relative"><input type="text" value={tempName} onChange={(e) => setTempName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-brand-500 focus:bg-white/10 transition-all font-bold" placeholder="Como quer ser chamado?" /><div className="absolute right-4 top-1/2 -translate-y-1/2 text-white/10"><Pencil className="w-4 h-4" /></div></div></div>
                    <div className="space-y-1 opacity-50"><label className="text-[9px] font-black text-white/30 uppercase ml-1 tracking-widest">E-mail (Login)</label><input type="text" disabled value={session.user.email} className="w-full bg-transparent border border-white/5 rounded-2xl px-5 py-4 text-xs text-white/60 cursor-not-allowed" /></div>
                  </div>
                  <div className="space-y-3 pt-2">
                    <button onClick={handleUpdateProfile} disabled={uploading} className="w-full bg-brand-600 text-white font-black py-5 rounded-2xl shadow-xl shadow-brand-600/20 active:scale-95 transition-all text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2">{uploading ? 'Sincronizando...' : <><Check className="w-4 h-4" /> Salvar Alterações</>}</button>
                    <button onClick={handleLogout} className="w-full bg-white/5 text-rose-500 font-black py-4 rounded-2xl hover:bg-rose-500/10 active:scale-95 transition-all text-[9px] uppercase tracking-[0.2em] flex items-center justify-center gap-2"><LogOut className="w-4 h-4" /> Encerrar Sessão</button>
                  </div>
                </div>
              </DialogContent>
            </Dialog>
